...
class AestheticCostFunction {
 public:
  ...
  bool operator()(const T* const parameters, T* residuals) const {
    ...
    Eigen::Vector3<T> camera_pos(parameters[0], T(0.0), parameters[1]);
    ...
    residuals[0] = (Eigen::Vector2<T>::Zero() -
                    Eigen::Vector2<T>(norm_pos.x(), norm_pos.y()))
                       .norm();
    return true;
  }
  ...
  static ceres::CostFunction* Create(std::weak_ptr<pxl::Camera> camera,
    ...
    ceres::AutoDiffCostFunction<AestheticCostFunction, 1, 2>* cost_function =
        new ceres::AutoDiffCostFunction<AestheticCostFunction, 1, 2>(
...
void AestheticCameraComponent::Update(float time_elapsed) {
  ...
  parameters[1] = camera_ptr->position.z();
  ...
  LOG(ERROR) << summary.FullReport();
  camera_ptr->position =
      Eigen::Vector3f(parameters[0], camera_ptr->position.y(), parameters[1]);
}
